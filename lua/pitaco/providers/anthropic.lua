...
function M.get_model()
    ...
    if vim.g.pitaco_anthropic_model_id_complained == nil then
        local message = "No model specified. Please set anthropic_model_id in the setup table. Using default value for now"
        vim.fn.confirm(message, "&OK", 1, "Warning")
...
function M.prepare_requests(messages)
    ...
    for i = 1, num_requests do
        ...
        local text = utils.prepare_code_snippet(buffer_number, starting_line_number, starting_line_number + split_threshold - 1)
...
function M.request(json_data)
    ...
    if response.status >= 400 then
        error("HTTP error: " .. tostring(response.body))
    end
